...
public enum Roles {
    ...
    //TODO: поправить описание: не все роли, стоящие выше, проходят проверку, ты писал,
    // что для этого есть метод hasPermissionToManageResourceType(), который уже проверяет и вышестоящих
    /**
    ...
    private boolean canBeAssignedToResourceType(ResourceType targetResourceType) {
        ...
        return getAllManagingResourceTypes().contains(targetResourceType);
    }
     ...
     * @return сет ресурсов
     */
    private Set<ResourceType> getAllManagingResourceTypes() {
        return INHERITED_ROLES.get(this).stream()
                .map(role -> role.managingResourceType)
